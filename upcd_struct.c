...
void writeUSBData(upcd_devInst *devInst, void *data, uint32_t len) {
	...
	uint32_t txSize, count;
	count = len;
	uint8_t *txData = (uint8_t *) data;

	while(count) {

	txSize = min(SIZEBUF, count);
	

       	USBEndpointDataPut(devInst->USBBase, devInst->intInEP, txData, txSize);

	USBEndpointDataSend(devInst->USBBase, devInst->intInEP, USB_TRANS_IN);
	 ...
	 USBEndpointDataSend(devInst->USBBase, 
			 devInst->intInEP, USB_TRANS_IN_LAST);
}
